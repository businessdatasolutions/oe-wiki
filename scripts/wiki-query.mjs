...
// Parameterized (template-friendly — no hardcoded wiki name):
//   1. WIKI_COLLECTION env var wins, else
//   2. derived from package.json "name", else
//   3. null -> query runs unscoped (safe fallback).
const COLLECTION =
  process.env.WIKI_COLLECTION ||
  (() => {
    try {
      return JSON.parse(readFileSync(join(REPO_ROOT, 'package.json'), 'utf8')).name || null;
    } catch {
      return null;
    }
  })();
...
for (let i = 0; i < hits.length; i++) {
  ...
  // Path format examples: "qmd://<collection>/concepts/foo.md", "wiki/concepts/foo.md", "concepts/foo.md"
